...
Deploy code:
    $ git add .
    $ git commit -am "Update 11.25"
    $ git push heroku master
See log:
    $ heroku logs -t --app uwiss
'''
...
def getGroupFromThreeGroups():
    ...
    if count_group1 == 0:
        ...
        if count_group1 / count_group2 == (4 / 2.25):
            ...
            if A/B > half ratio,
                if B/C < half ratio, return B 
                else, return C  
            else,
                return A
            '''
            if count_group1 / count_group2 > (3.7 / 2.25):  
...
def selectAdvisor(taskName, group):
    ...
    print('selected advisor: ' + selected.advisor_name)
...
def newtask(request):
    '''
    taskChoices = []
    taskObj = task.objects.order_by("task_name")
    for each in taskObj:
        taskChoices.append(each.task_name)
    '''
    context = {
        "form": newTaskForm,
...
def calcAdvisor(request, taskName, advisorName):
    ...
    tt_list = turnStrToList(taskHandler(taskName).task_advisor_list)
    if len(tt_list) > 0 and tt_list[0] != None and advisorHandler(tt_list[0]) != None \
            and advisorHandler(tt_list[0]).advisor_group == 4:
        advv = advisorHandler(tt_list[0])
        messages.add_message(request, messages.INFO, '%s is assigned to: %s. %s' % (
            taskName, advv.advisor_name, taskHandler(taskName).task_requirement))
        ...
        assignTo.save()
        thisTask.save()
        #print('selected advisor: ' + assignTo.advisor_name)
        print('special task ' + taskName)

    elif advisorName == 'notspecified':
        hasGroup1 = False
        hasGroup2 = False
        hasGroup3 = False
        for eachTask in task.objects.all():
        ...
        if taskNotAssigned != False:
            print('An advisor is specified to handle this task')
...
def summary(request):
    advisor_list = advisor.objects.order_by('advisor_name')
    ...
    for eachAdvisor in advisor_list:
        ...
        if eachAdvisor.advisor_group == 1:
            group1_sum += eachAdvisor.advisor_count
        elif eachAdvisor.advisor_group == 2:
            group2_sum += eachAdvisor.advisor_count
        elif eachAdvisor.advisor_group == 3:
            group3_sum += eachAdvisor.advisor_count
    ...
    context = {
        ...
        "group2_sum": group2_sum,
        "group3_sum": group3_sum,
    }
...
def addCounter(request):
    ...
    if form.is_valid():
        ...
        messages.add_message(request, messages.SUCCESS, "Traffic updated successfully.")
