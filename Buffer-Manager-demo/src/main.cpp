...
void testBufMgr()
{
  ...
  try
	...
	// File file7 = File::create(filename5);
...
void test3()
{
	try
	{	
		// Try to read a file that doesn't yet exist
...
void test7()
{
	// Delete pages which is not allocated yet, should generated exception
	try {
		for (i = 1; i <= num; i++) {
			bufMgr->disposePage(file1ptr, i);
			PRINT_ERROR("ERROR : Should handle HashNotFoundException.");
		}
	}
...
void test8()
{
	try
	{
		// This is reverse of test4, after unpin the page,
		// we should be able to flushFile without exception
		for (i = 0; i < num; i++)
		{
			bufMgr->allocPage(file1ptr, pid[i], page);
			sprintf((char*)tmpbuf, "test.1 Page %d %7.1f", pid[i], (float)pid[i]);
	...
	{
		PRINT_ERROR("ERROR : Exception should not be generated at this point.");
	}
	catch(PagePinnedException e)
	{
		PRINT_ERROR("ERROR : Exception should not be generated at this point.");
